...
# ── DATA PATH ──────────────────────────────────────────────────────────────────
DATA_PATH = os.path.join(os.path.dirname(os.path.abspath(__file__)), "HousingCentersOCFinalData.xlsx")
DATA_SHEET = "Short"

# ── LOAD DATA ──────────────────────────────────────────────────────────────────
@st.cache_data
def load_data(filepath, sheet_name=DATA_SHEET):
    import openpyxl
    wb = openpyxl.load_workbook(filepath, data_only=True)
    ws = wb[sheet_name]

    # Headers live on row 1 in this workbook. 'Project Type' appears twice —
    # the second copy is actually the housing subtype (e.g. Permanent
    # Supportive Housing, Income-Restricted), so rename it on the way in.
    raw_headers = [cell.value for cell in ws[1]]
    headers, seen_project_type = [], 0
    for h in raw_headers:
        if h == 'Project Type':
            seen_project_type += 1
            headers.append('Project Type' if seen_project_type == 1 else 'Housing Subtype')
        else:
            headers.append(h)

    data = [row for row in ws.iter_rows(min_row=2, values_only=True)]
    df = pd.DataFrame(data, columns=headers)
    df = df.loc[:, df.columns.notna()]              # drop trailing blank columns
    df = df.loc[:, ~df.columns.duplicated()].copy()

    numeric_cols = [
        'Total Units', 'Affordable Units', 'PSH Bed',
        'Rent Burden %', 'Median HH Income Percentile',
    ...
    df = df.dropna(subset=['Property Name', 'City'], how='all').reset_index(drop=True)

    # Collapse the population-served flag columns into one readable field
    pop_cols = [c for c in ['Family', 'Special Needs', 'Veterans', 'Seniors', 'Individuals'] if c in df.columns]
    def combine_population(row):
        served = [c for c in pop_cols if pd.notna(row[c])]
        return ', '.join(served) if served else 'General / Unspecified'
    df['Population Served'] = df[pop_cols].apply(combine_population, axis=1) if pop_cols else 'General / Unspecified'

    df['Housing Subtype'] = df['Housing Subtype'].fillna('Standard Affordable').replace({
        'Permanent Supportive Housing ': 'Permanent Supportive Housing',
        'Income- Restricted': 'Income-Restricted',
    }) if 'Housing Subtype' in df.columns else 'Standard Affordable'
    ...
    for col in key_vars + ['Affordable Units']:
    ...
    tract_agg = enriched_clean.groupby(['Census Tract', 'City']).agg(
        Affordable_Units=('Affordable Units', 'sum'),
        Total_Units=('Total Units', 'sum'),
        Num_Programs=('Property Name', 'count'),
        ...
        Housing_Subtypes=('Housing Subtype', lambda x: ', '.join(sorted(set(x.dropna()))))
    ).reset_index().round(3)

    med_units = tract_agg['Affordable_Units'].median()
    med_need = tract_agg['Need_Score'].median()
    tract_agg['Underserved'] = (
        (tract_agg['Need_Score'] > med_need) & (tract_agg['Affordable_Units'] < med_units)
    )
...
st.sidebar.markdown("---")
st.sidebar.markdown(f"**Properties:** {len(df):,}")
st.sidebar.markdown(f"**Affordable Units:** {int(df['Affordable Units'].sum()):,}")
...
st.sidebar.markdown("**Data:** OC Affordable Housing Inventory · CalEnviroScreen 4.0/5.0 · Census ACS")
...
if page == "📊 Overview":
    ...
    with c1:
        st.markdown(f"""<div class="metric-card">
            ...
            <div class="metric-label">Total Affordable Housing Properties</div>
        </div>""", unsafe_allow_html=True)
    with c2:
        st.markdown(f"""<div class="metric-card">
            <div class="metric-value">{int(df['Affordable Units'].sum()):,}</div>
            <div class="metric-label">Affordable Units Across OC</div>
        </div>""", unsafe_allow_html=True)
    with c3:
        st.markdown(f"""<div class="metric-card">
    ...
    with c2:
        st.subheader("Affordable Units vs. Need Score by Tract")
        fig = px.scatter(tract_agg, x='Need_Score', y='Affordable_Units',
                         color='Underserved',
                         color_discrete_map={True: '#D95A00', False: '#1A5276'},
                         hover_data=['Census Tract', 'City'],
                         labels={'Need_Score': 'Need Score', 'Affordable_Units': 'Affordable Units'},
                         size='Affordable_Units', size_max=40)
        fig.add_hline(y=tract_agg['Affordable_Units'].median(), line_dash="dash",
                      line_color="gray", opacity=0.5, annotation_text="Median units",
                      annotation_font_color="#1A2B3C")
        fig.add_vline(x=tract_agg['Need_Score'].median(), line_dash="dash",
    ...
    _rb_units_r = enriched['Rent Burden %'].corr(enriched['Affordable Units'])
    st.markdown(f"""<div class="insight-box">
        <strong>Key Finding:</strong> Areas with higher housing need scores tend to have fewer
        affordable units — the correlation between rent burden and affordable unit count is
        {_rb_units_r:.3f}, {"confirming resources are misaligned with community need." if _rb_units_r < 0 else "suggesting supply is at least loosely tracking rent burden."}
    </div>""", unsafe_allow_html=True)

    st.subheader("Population Served Across OC")
    pop_counts = (
        df['Population Served'].str.split(', ').explode().value_counts().reset_index()
    )
    pop_counts.columns = ['Population Served', 'Count']
    fig = px.bar(pop_counts, x='Population Served', y='Count',
...
# ── MAP ────────────────────────────────────────────────────────────────────────
    ...
    st.markdown("Each circle = one census tract. **Size** = affordable units. **Color** = composite need score.")
    st.markdown("---")
    ...
    for _, row in filtered.iterrows():
        ...
        rad = max(8, min(40, row['Affordable_Units'] / 10))
        mapped += 1
        ...
        <div style="font-family:Arial;width:240px;line-height:1.5;color:#1A2B3C">
            ...
            <table style="width:100%;font-size:12px">
                ...
                <tr><td><b>Affordable Units</b></td><td>{int(row['Affordable_Units'])}</td></tr>
        ...
        folium.CircleMarker(
            ...
            tooltip=f"Tract {tract} | Need: {row['Need_Score']:.3f} | Units: {int(row['Affordable_Units'])}"
        ).add_to(lg_need)
    ...
    if layer_programs:
        ...
        for _, prog in prog_df.iterrows():
            ...
            if t and t in TRACT_COORDS:
                ...
                random.seed(hash(str(prog.get('Property Name', ''))))
                jlat = plat + random.uniform(-0.008, 0.008)
                jlon = plon + random.uniform(-0.008, 0.008)
                units = prog.get('Affordable Units', 0)
                subtype = prog.get('Housing Subtype', 'Standard Affordable')
                pin_col = {
                    'Permanent Supportive Housing': 'blue',
                    'Income-Restricted': 'green',
                    'Standard Affordable': 'darkblue',
                }.get(subtype, 'gray')
                folium.CircleMarker(
                    location=[jlat, jlon], radius=4,
                    color='white', fill=True, fill_color=pin_col, fill_opacity=0.9, weight=1.5,
                    tooltip=f"{prog.get('Property Name','')} | {subtype} | {int(units) if units==units else '?'} affordable units",
                    popup=folium.Popup(
                        f"<b>{prog.get('Property Name','')}</b><br>"
                        f"Type: {subtype}<br>City: {prog.get('City','')}<br>"
                        f"Population Served: {prog.get('Population Served','')}<br>"
                        f"Affordable Units: {int(units) if units==units else 'Unknown'}",
                        max_width=220)
                ).add_to(lg_programs)
    ...
    if layer_programs:
        st.markdown("**Program pins:** 🔵 Permanent Supportive Housing · 🟢 Income-Restricted · 🔷 Standard Affordable")

# ── UNDERSERVED ────────────────────────────────────────────────────────────────
elif page == "🚨 Underserved Tracts":
    st.title("🚨 Critically Underserved Tracts")
    st.markdown("Above-median need + below-median affordable units = most urgent gaps in OC's housing safety net.")
    st.markdown("---")
    ...
    for _, row in underserved.iterrows():
        ...
        <div class="alert-box">
            ...
            <div style="display:flex;gap:24px;margin-top:10px;flex-wrap:wrap">
                ...
                <div><b>Affordable Units:</b> {int(row['Affordable_Units'])}</div>
    ...
    st.markdown("---")
    fig = px.bar(underserved, x='Census Tract', y=['Need_Score', 'Affordable_Units'],
...
# ── CITY ANALYSIS ──────────────────────────────────────────────────────────────
    ...
    with c2:
        st.subheader("Affordable Units by City")
        cb  = city_f.sort_values('Affordable_Units', ascending=True)
        fig = go.Figure(go.Bar(x=cb['Affordable_Units'], y=cb['City'],
                               orientation='h', marker_color='#1A5276'))
        fig.update_layout(xaxis_title='Affordable Units', height=max(300, len(cb) * 28))
        st.plotly_chart(style_fig(fig), use_container_width=True)

    st.subheader("Need vs. Affordable Units — City Scatter")
    fig = px.scatter(city_f, x='Avg_Need_Score', y='Affordable_Units',
    ...
    disp.columns = ['City', 'Affordable Units', 'Avg Need Score', '# Programs', 'Avg Rent Burden', 'Avg Poverty %ile']
...
# ── CORRELATIONS ───────────────────────────────────────────────────────────────
    ...
    corr_cols   = ['Rent Burden %', 'Income Need', 'Poverty Line Percentile', 'Affordable Units']
    corr_labels = ['Rent Burden', 'Income Need', 'Poverty %ile', 'Affordable Units']
    ...
    r_income_poverty = corr_matrix.loc['Income Need', 'Poverty Line Percentile']
    r_rent_income    = corr_matrix.loc['Rent Burden %', 'Income Need']
    r_rent_units     = corr_matrix.loc['Rent Burden %', 'Affordable Units']
    ...
    c1, c2 = st.columns(2)
    with c1:
        st.markdown(f"""<div class="insight-box"><b>Income Need &amp; Poverty (r={r_income_poverty:.3f}):</b>
          {_strength(r_income_poverty)} — both measure economic deprivation from different angles.</div>""",
                    unsafe_allow_html=True)
        st.markdown(f"""<div class="insight-box"><b>Rent Burden &amp; Income Need (r={r_rent_income:.3f}):</b>
          {_strength(r_rent_income)} — rent stress is partly but not fully income-driven.</div>""",
                    unsafe_allow_html=True)
    with c2:
        st.markdown(f"""<div class="insight-box"><b>Rent Burden &amp; Affordable Units (r={r_rent_units:.3f}):</b>
          {"Negative — highest need areas tend to have fewer affordable units." if r_rent_units < 0 else "Positive — affordable unit supply loosely tracks rent burden."}</div>""",
                    unsafe_allow_html=True)
...
# ── EXPLORE DATA ───────────────────────────────────────────────────────────────
    ...
    with c2:
        sel_subtype = st.selectbox("Housing Subtype", ["All"] + sorted(df['Housing Subtype'].dropna().unique().tolist()))
    with c3:
        min_units_val = st.number_input("Min Affordable Units", min_value=0, value=0, step=5)

    fdf = df.copy()
    if sel_city != "All":
        fdf = fdf[fdf['City'] == sel_city]
    if sel_subtype != "All":
        fdf = fdf[fdf['Housing Subtype'] == sel_subtype]
    fdf = fdf[pd.to_numeric(fdf['Affordable Units'], errors='coerce').fillna(0) >= min_units_val]

    st.markdown(f"**{len(fdf)} properties** match your filters")
    disp_cols = ['Property Name', 'City', 'Housing Subtype', 'Population Served',
                 'Total Units', 'Affordable Units', 'Census Tract', 'Rent Burden %', 'Poverty Line Percentile']
...
|--------|-----------|-------------|
| OC Affordable Housing Inventory | Property name, subtype, units, city, tract | {len(df)} OC affordable housing properties |
| CalEnviroScreen 4.0 / 5.0 | Environmental burden %ile | CA EPA cumulative burden scores |
...
CalEnviroScreen (4.0 in the property table, 5.0 on the interactive map layer) is shown
as a separate map toggle layer only. Excluded from the formula to avoid double-counting
...
- Affordable Units < **{med_units:.0f}** (dataset median)
    """)

    enriched_pct = len(enriched) / len(df) * 100
    st.subheader("Limitations")
    st.markdown(f"""
- Only {len(enriched)} of {len(df)} properties ({enriched_pct:.0f}%) have complete census tract + need data
- Dataset covers **affordable housing properties only** — homeless-services inventory (shelters, PSH beds, rapid re-housing) from earlier versions of this dashboard is not included
- Static snapshot — housing need and unit counts change over time
- Affordable unit counts do not reflect vacancy rates or waitlists
- Property coordinates are census tract centroids (approximate) with random jitter, not exact addresses
